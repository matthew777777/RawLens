...
- `ForegroundServiceDidNotStartInTimeException` death while a JPEG develop is outstanding: the service now declares its `specialUse` type explicitly on API 34+, keeps acknowledgement as the first main-thread work (diagnostics moved off the main thread), and stops itself instead of crashing when the promotion is rejected, so the save finishes at background priority. The ordered stop also falls back to a direct stop when background delivery is rejected instead of leaking a foreground service
...
- Silent RAW video in motioncam-decoder tooling: the writer emitted one motion chunk per sensor drain (~100/s, 318 gyro + 313 accel chunks for a 95-frame take) while motioncam-decoder rejects files with more motion chunks than frames + 1 — `Invalid gyro index` aborted the whole open, hiding the intact PCM audio too. The native writer now buffers motion and coalesces it to at most one chunk per sensor per committed frame plus a trailing chunk at close (same fix upstream in MediaCinemaRAW-Encoder); `RawVideoRecordTest` pins the bound on-device, `tools/mcraw/motion_coalesce.cpp` pins it on-host against both decoders, and `tools/mcraw/repair_motion.py` rewrites pre-fix takes with sample-identical frames/audio/motion that motioncam-decoder opens
- Silent RAW video in MotionCam Tools despite a valid container: absolute boot-time timestamps overflow 32-bit ms math in some readers, breaking audio sync entirely (PhotonCamera's documented finding; their relative-timestamp takes are heard in the same app). Every on-disk timestamp is now recording-relative to the first queued frame — frames, audio, motion, and the timestamp/filename/metadataTimestamp frame-metadata strings — with pre-roll audio/motion without video dropped so all timelines start together; `RawVideoRecordTest` pins the first frame near ts 0 on-device, and `repair_motion.py` applies the same rebase plus motion coalescing to old takes
- RAW video records stereo-first audio with mono fallback (PhotonCamera and motioncam-decoder's own fixture use `audioChannels: 2`); the declared count is the actual recorded count. The container path was already channel-agnostic (verbatim int16 arrays). `tools/mcraw/interop.cpp` pins a distinct-L/R stereo round-trip through both decoders, and `tools/mcraw/repair_motion.py --stereo` converts old mono takes to L=R stereo
- Silent RAW video in MotionCam Tools despite valid audio (the actual silence fix): Tools v1.0 predates gyro/accelerometer support and its tail scan stops at the first motion item, so trailing motion data before the audio index made it report "No audio chunks found" (proven from the app's own fuse.txt log plus decoder archaeology; the Jan-2026 parser finds 0 chunks in pre-fix files, all chunks in fixed ones). Both writers now emit the audio index before flushing trailing motion — no motion data sits between the last frame and the audio index — pinned by `tools/mcraw/legacy_audio.cpp` against the Jan-2026 parser, structurally by `EncoderTests`/`RawVideoRecordTest`, with `repair_motion.py` rewriting old takes in the fixed order
