...
/** ADB-only validation of the production shader, imports and DNG metadata writer. */
class DualRawMergeInstrumentedTest {
    @Test fun adjacentRawPair() {
        ...
        val outDir = File(context.getExternalFilesDir(null),"dualraw-validation").apply { mkdirs() }
        ...
        try {
            val status = DualRawVulkan.init(context.assets.open("shaders/vf/vf_superpixel.spv").use { it.readBytes() },
            ...
            for((index,item) in captured.withIndex()) {
                ...
                check(r[CaptureResult.SENSOR_SENSITIVITY]==100)
            ...
            for(pairIndex in 0..1) {
                ...
                val measuredWhite=(lr[CaptureResult.SENSOR_DYNAMIC_WHITE_LEVEL] ?: c[CameraCharacteristics.SENSOR_INFO_WHITE_LEVEL]!!).toFloat()
                val lw=if(pairIndex==1) 128f else measuredWhite // exercise clipping on ordinary scenes
                ...
                // One common reference scale with reserved HDR headroom.
                val ratio=max(1f,(0..3).maxOf { scale*(hw-hb[it])/(lw-lb[it]) })
                ...
                low.hardwareBuffer!!.use { la -> high.hardwareBuffer!!.use { ha ->
                    val code=DualRawVulkan.merge(la,ha,intArrayOf(size.width,size.height,lp.rowStride/2,hp.rowStride/2),params,output,timings)
                ...
                for(y in 0 until size.height step 7) for(x in 0 until size.width step 7) {
                    val site=(y%2)*2+x%2
                    val rawLong=a.getShort(y*lp.rowStride+x*2).toInt() and 65535
                    val rawShort=b.getShort(y*hp.rowStride+x*2).toInt() and 65535
                    val value=if(rawLong<lw) (rawLong-lb[site])/(lw-lb[site])
                        else if(rawShort>=hw) ratio else (rawShort-hb[site])*scale/(lw-lb[site])
                    val expected=(value/ratio*64511f+1024f).roundToInt().coerceIn(0,65535)
                ...
                val changed=ByteBuffer.allocateDirect(size.width*size.height*2).order(ByteOrder.LITTLE_ENDIAN)
                val perturbed=params.copyOf()
                repeat(4) { perturbed[4+it]+=100f }
                low.hardwareBuffer!!.use { la -> high.hardwareBuffer!!.use { ha ->
                    check(DualRawVulkan.merge(la,ha,intArrayOf(size.width,size.height,lp.rowStride/2,hp.rowStride/2),
                        perturbed,changed,DoubleArray(3))==0)
                } }
                var protected=0
                for(y in 0 until size.height) for(x in 0 until size.width) {
                    val raw=a.getShort(y*lp.rowStride+x*2).toInt() and 65535
                    if(raw<lw) {
                        val offset=(y*size.width+x)*2
                        check(output.getShort(offset)==changed.getShort(offset)) { "Short contaminated long at $x,$y" }
                        protected++
                    }
                }
                check(protected>0)
                log("INVARIANCE protectedLongPixels=$protected shortBlackPerturbation=100DN exact=true")
                val file=File(outDir,"RawLens_Vulkan_adjacent_pair$pairIndex.dng")
                DngCreator(c,lr).use { creator ->
                    creator.setOrientation(1)
                    creator.setDescription("RawLens production merge validation; pair 1 uses a synthetic long clipping threshold")
                    output.position(0)
                    file.outputStream().use { creator.writeByteBuffer(it,size,output,0) }
                }
                DualRawDngMetadata.patch(file,ratio.toDouble())
