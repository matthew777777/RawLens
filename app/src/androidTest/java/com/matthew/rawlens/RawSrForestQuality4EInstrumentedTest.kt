...
class RawSrForestQuality4EInstrumentedTest {
    @Test fun realForestMergeQualityAtBurstCounts() {
        ...
        Gles31RawSrProcessor(instrumentation.targetContext).use { processor ->
            ...
            processor.processPacked(refFrames, config, referenceOnly = true,
                ...
                // Covariance parity tap: GPU packed precision vs CPU oracle precision.
                val cov = requireNotNull(gpuCov) { "no covariance texture" }
                val cpuPrec = RawSrKernelCovariance.precision(
                    RawSrCovarianceGuide.guide(refFrames.first()).gray, refTuning)
            ...
            for (count in listOf(2, 8, 15, 30)) {
                ...
                if (count == 30 && frameCov.size == frames.size) {
                    ...
                    for ((i, frame) in frames.withIndex()) {
                        val cpu = RawSrKernelCovariance.precision(
                            RawSrCovarianceGuide.guide(frame).gray, tuning).values
                        val gpu = frameCov[i]
                        var mx = 0f; var vv = 0
     ...
     * distances differ by less than [TIE_BAND]. Only meaningful where rgb
     * derives from the nearest totals, i.e. at fallback pixels. Ported from
     * the sea 4E gate. */
