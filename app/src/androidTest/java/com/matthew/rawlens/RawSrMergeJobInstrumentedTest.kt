...
class RawSrMergeJobInstrumentedTest {
    ...
    @Test fun gpuMergeStripedReadbackMatchesFullReadback() {
        // Cross the 256-row readback boundary and exercise a partial final band.
        val w = 64; val h = 270
        val ref = packed(w, h, seed = 7)
        val mov = packed(w, h, seed = 7)
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        VkRawSrProcessor(context).use { processor ->
            processor.processPacked(listOf(ref.first, mov.first)) { output ->
                ...
                // Ragged bands with a short tail; each band must equal the
                // corresponding slice of the whole-frame repack.
                val actual = FloatArray(wOut * hOut * 3)
                var y = 0
                var step = 0
                while (y < hOut) {
                    val rows = minOf(if (step % 2 == 0) 5 else 9, hOut - y)
                    RawSrMergeJob.readMergedRgbStrip(
                        output.mergedTextureId, wOut, hOut, y, rows, actual, y * wOut * 3
                    )
                    y += rows
                    step++
                }
                assertArrayEquals(expected, actual, 0f)
                // The striped DNG stream serializes exactly the whole-write bytes.
                ...
                val whole = ByteArrayOutputStream().also {
                    LinearRgbDngWriter.write(
                        it, MergedLinearRgb(wOut, hOut, expected), ref.second, provenance
                    )
                }.toByteArray()
                val striped = ByteArrayOutputStream().also {
                    LinearRgbDngWriter.writeStriped(
                        it, wOut, hOut, ref.second, provenance, stripRows = 5
                    ) { startY, rows, band ->
                        RawSrMergeJob.readMergedRgbStrip(
                            output.mergedTextureId, wOut, hOut, startY, rows, band
                        )
                    }
                }.toByteArray()
                ...
                assertThrows(IllegalArgumentException::class.java) {
                    RawSrMergeJob.readMergedRgbStrip(
                        output.mergedTextureId, wOut, hOut, 0, hOut + 1,
                        FloatArray(wOut * (hOut + 1) * 3)
                    )
                }
                assertThrows(IllegalArgumentException::class.java) {
                    RawSrMergeJob.readMergedRgbStrip(
                        output.mergedTextureId, wOut, hOut, 0, 1, FloatArray(2)
                    )
                }
            }
        }
