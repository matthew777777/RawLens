...
// One invocation per RAW pixel on the full-res code plane. Flags stuck-bright
...
//
// Gate mirrors RawSrHotPixel.detect exactly: hot when the tap clears the
// gate against BOTH its ring mean and its ring maximum (a stuck-high tap is
// the local maximum of its colour plane; texture and step edges routinely
// clear a mean-only gate on one side). Bright-only; an invalid model
...
// flagged. Output is 1 (hot) or 0 in an R32UI texture.
...
const float HOT_ABS_FLOOR = 0.02;
    ...
    if (u_model_valid != 0) {
        ...
        if (finite(code) && finite(variance) && variance > 0.0) {
            ...
            if (count >= MIN_RING_TAPS) {
                ...
                if (code - sum / float(count) > gate && code - peak > gate) outMask = 1u;
            }
        }
    }
