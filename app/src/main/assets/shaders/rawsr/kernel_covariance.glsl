...
// Mirrors RawSrKernelCovariance (Jamy-L Alg. 5, linear selection law): separable 2x2
// gradients, 2x2 structure-tensor window, analytic eigendecomposition, kernel radii
// from the resolved tuning, packed 2x2 precision output. One invocation per quad
// pixel; no per-pixel host objects. Packing matches the SkyKing alterCov consumer:
...
void main() {
    ...
    if (finite(t00) && finite(t01) && finite(t11)) {
        float b = -(t00 + t11);
        ...
        float axis1 = (2.0 - anisotropy) + (anisotropy - 1.0) / u_k_shrink;
        float axis2 = (2.0 - anisotropy) + (anisotropy - 1.0) * u_k_stretch;
