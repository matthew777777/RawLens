#version 450
// generated by tools/srvk_shader_transform.py from rawsr/merge_accumulate.glsl (sources stay ESSL)
layout(binding = 14, std140) uniform SrUniforms {
    highp uvec3 u_dispatch_offset; // src:2
    ivec2 u_size; // src:49
    ivec2 u_tile_grid; // src:50
    int u_tile_size; // src:51
    float u_motion_edge; // src:52
    ivec2 u_guide_size; // src:53
    ivec4 u_fc; // src:54
    int u_is_reference; // src:55
    int u_use_r; // src:56
    int u_use_chroma; // src:57
    float u_green_noise_s; // src:58
    float u_green_noise_o; // src:59
};
...
layout(binding = 2) uniform sampler2D u_precision;
layout(binding = 3) uniform sampler2D u_r;
layout(binding = 4) uniform sampler2D u_num;
layout(binding = 5) uniform sampler2D u_den;
layout(binding = 6) uniform sampler2D u_oob;
layout(binding = 7) uniform sampler2D u_near_num;
layout(binding = 8) uniform sampler2D u_near_den;
layout(binding = 9, rgba32f) writeonly uniform highp image2D img_num;
layout(binding = 10, rgba32f) writeonly uniform highp image2D img_den;
layout(binding = 11, r32f) writeonly uniform highp image2D img_oob;
layout(binding = 12, rgba32f) writeonly uniform highp image2D img_near_num;
layout(binding = 13, rgba32f) writeonly uniform highp image2D img_near_den;
