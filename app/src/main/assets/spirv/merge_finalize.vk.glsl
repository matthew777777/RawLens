#version 450
// generated by tools/srvk_shader_transform.py from rawsr/merge_finalize.glsl (sources stay ESSL)
layout(binding = 9, std140) uniform SrUniforms {
    highp uvec3 u_dispatch_offset; // src:2
    ivec2 u_size; // src:30
    float u_min_support; // src:31
    vec4 u_highlight_neutral; // src:32
};
...
layout(binding = 0) uniform sampler2D u_num;
layout(binding = 1) uniform sampler2D u_den;
layout(binding = 2) uniform sampler2D u_ref_num;
layout(binding = 3) uniform sampler2D u_ref_den;
layout(binding = 4) uniform sampler2D u_near_num;
layout(binding = 5) uniform sampler2D u_near_den;
layout(binding = 6) uniform sampler2D u_rc;
layout(binding = 7, rgba32f) writeonly uniform highp image2D img_out;
layout(binding = 8, r32f) writeonly uniform highp image2D img_fallback;
