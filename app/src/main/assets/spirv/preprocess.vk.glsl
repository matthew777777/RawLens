#version 450
// generated by tools/srvk_shader_transform.py from raw/preprocess.glsl (sources stay ESSL)
layout(binding = 3, std140) uniform SrUniforms {
    highp uvec3 u_dispatch_offset; // src:2
    ivec2 u_size; // src:17
    ivec2 u_sensor_origin; // src:18
    ivec4 u_fc; // src:19
    vec4 u_black; // src:20
    float u_white; // src:21
    ivec2 u_lens_size; // src:22
    ivec4 u_active; // src:23
    int u_apply_lens; // src:24
};
...
layout(binding = 0) uniform highp usampler2D u_raw;
layout(binding = 1) uniform sampler2D u_lens;
layout(binding = 2, r32f) writeonly uniform highp image2D img_out;
