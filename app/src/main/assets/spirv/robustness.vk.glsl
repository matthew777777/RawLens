#version 450
// generated by tools/srvk_shader_transform.py from rawsr/robustness.glsl (sources stay ESSL)
layout(binding = 8, std140) uniform SrUniforms {
    highp uvec3 u_dispatch_offset; // src:2
    ivec2 u_size; // src:25
    ivec2 u_tile_grid; // src:26
    int u_tile_size; // src:27
    float u_t; // src:28
    float u_s1; // src:29
    float u_s2; // src:30
    float u_mth_quad; // src:31
    float u_max_residual; // src:32
    vec3 u_mov_alpha; // src:33
    vec3 u_mov_beta; // src:34
    int u_ref_valid; // src:35
    int u_mov_valid; // src:36
    int u_lut_bins; // src:41
    int u_lut_enabled; // src:42
};
...
layout(binding = 0) uniform sampler2D u_ref_lin;
layout(binding = 1) uniform sampler2D u_mov_lin;
...
layout(binding = 3) uniform highp usampler2D u_hot_ref;
layout(binding = 4) uniform highp usampler2D u_hot_mov;
...
layout(binding = 5) uniform sampler2D u_lut;
layout(binding = 6, r32f) writeonly uniform highp image2D img_r;
layout(binding = 7, r32ui) writeonly uniform highp uimage2D img_flags;
