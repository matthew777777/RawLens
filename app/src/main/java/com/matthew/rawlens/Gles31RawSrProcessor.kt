...
class Gles31RawSrProcessor(context: Context) : Closeable {
     ...
     */
    private fun uploadNoiseLut(lut: RawSrNoiseLut.Lut?, active: Session, arena: Arena): Gles31AmazeProcessor.GlTexture {
        if (lut == null) {
    ...
    private fun robustnessPass(refLinear: Gles31AmazeProcessor.GlTexture,
                               ...
                               lutTexture: Gles31AmazeProcessor.GlTexture,
                               refHot: Gles31AmazeProcessor.GlTexture,
                               movHot: Gles31AmazeProcessor.GlTexture,
     ...
     */
    private fun hotMask(codes: Gles31AmazeProcessor.GlTexture, params: RawSrRobustness.GpuParams,
                        active: Session, arena: Arena): Gles31AmazeProcessor.GlTexture {
     ...
     * the mask.
     */
    private fun hotInpaint(cfa: Gles31AmazeProcessor.GlTexture, mask: Gles31AmazeProcessor.GlTexture,
                           pattern: BayerPattern, active: Session, arena: Arena): Gles31AmazeProcessor.GlTexture {
    ...
    private fun unblockerPass(
        gray: Gles31AmazeProcessor.GlTexture,
        r: Gles31AmazeProcessor.GlTexture,
        flags: Gles31AmazeProcessor.GlTexture,
        params: RawSrRobustness.GpuParams,
        active: Session,
        arena: Arena
    ): Triple<Gles31AmazeProcessor.GlTexture, Gles31AmazeProcessor.GlTexture, Gles31AmazeProcessor.GlTexture> {
    ...
    private fun accumulateRc(rc: Gles31AmazeProcessor.GlTexture, r: Gles31AmazeProcessor.GlTexture,
                             out: Gles31AmazeProcessor.GlTexture, active: Session) {        active.pass("rawsr/robustness_accumulate.glsl") {
    ...
    private fun kernelNetTexture(
        ...
        active: Session,
        arena: Arena
    ): Gles31AmazeProcessor.GlTexture? {
    ...
    private class Arena : Closeable {
        ...
        override fun close() {
            // No glFinish: the consumer already drained whatever it read
            // (readback is the sink), GL defers texture deletion until the
            // last queued user completes, and command order already sequences
            // this execute before the next — a terminal drain is pure idle
            // for CPU and GPU alike (PhotonCamera PR 193, efa6e08a
            // "amaze: drop the terminal glFinish", same rationale).
    ...
    private fun ensureSession(): Session {
        ...
        return session ?: Session(Gles31AmazeProcessor.EglComputeContext(),
            Gles31AmazeProcessor.ProgramCache(appContext, slicedDispatch = true), Gles31AmazeProcessor.UploadBuffers())
    ...
    private data class Session(
        ...
        fun pass(asset: String, block: Bound.() -> Unit) = Bound(programs.get(asset), asset).block()
    }

    private class Bound(private val program: Int, private val asset: String) {
        private var unit = 0
        ...
        fun vec2(name: String, x: Float, y: Float) = GLES31.glUniform2f(location(name), x, y)
        ...
        fun dispatch(width: Int, height: Int, localX: Int, localY: Int) {
            val offset = location("u_dispatch_offset")
            val budget = if (asset.endsWith("block_match.glsl") || asset.endsWith("lk_refine.glsl")) 64 else 131072
            for (slice in RawSrGpuScheduling.slices(width, height, localX, localY, budget)) {
                GLES31.glUniform3ui(offset, slice.x, slice.y, 0)
                GLES31.glDispatchCompute(slice.groupsX, slice.groupsY, 1)
                GLES31.glMemoryBarrier(GLES31.GL_SHADER_IMAGE_ACCESS_BARRIER_BIT or GLES31.GL_TEXTURE_FETCH_BARRIER_BIT)
                check(GLES31.glGetError() == GLES31.GL_NO_ERROR) { "RAW-SR GLES dispatch failed: $asset" }
                // A timeout is a failed merge, never permission to grow the GPU queue.
                val fence = GLES31.glFenceSync(GLES31.GL_SYNC_GPU_COMMANDS_COMPLETE, 0)
                check(fence != 0L) { "RAW-SR fence allocation failed" }
                try {
                    val result = GLES31.glClientWaitSync(fence, GLES31.GL_SYNC_FLUSH_COMMANDS_BIT, 10_000_000_000L)
                    check(result == GLES31.GL_ALREADY_SIGNALED || result == GLES31.GL_CONDITION_SATISFIED) {
                        "RAW-SR GPU did not complete $asset: 0x${result.toString(16)}"
                    }
                } finally {
                    GLES31.glDeleteSync(fence)
                }
                // Leave a submission opportunity for the independent RAW VF context.
                Thread.sleep(1)
            }
        }
