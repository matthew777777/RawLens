...
object RawSrRobustness {
    ...
    fun linearGuide(frame: RawSrPackedFrame): LinearGuide {
        ...
        // phases once (every quad holds one R, two G, one B sample).
        var channelSealed = false
        for (qy in 0 until outHeight) for (qx in 0 until outWidth) {
            var r = 0f
            var g = 0f
            var b = 0f
            var greens = 0
            ...
            for (i in 0..1) for (j in 0..1) {
                ...
                if (modelValid) {
                    ...
                    if (code > white - RAIL_SIGMA * sigma || code < black + RAIL_SIGMA * sigma) quadRail = true
                }
                when (input.normalization.sensorPattern.colorAt(sx, sy)) {
                    CfaColor.RED -> {
                        r = value; aR = tables.alpha[phase]; bR = tables.beta[phase]
                    }
                    CfaColor.GREEN -> {
                        g += value; aG += tables.alpha[phase]; bG += tables.beta[phase]; greens++
                    }
                    CfaColor.BLUE -> {
                        b = value; aB = tables.alpha[phase]; bB = tables.beta[phase]
                    }
                }
            }
            ...
            if (!channelSealed) {
                channelSealed = true
                alpha[0] = aR.toFloat(); alpha[1] = (aG * 0.5).toFloat(); alpha[2] = aB.toFloat()
                beta[0] = bR.toFloat(); beta[1] = (bG * 0.5).toFloat(); beta[2] = bB.toFloat()
            }
        }
        return LinearGuide(outWidth, outHeight, red, green, blue, alpha, beta, model, rail)
    }
    ...
    fun evaluate(
        ...
        for (y in 0 until height) for (x in 0 until width) {
            ...
            val tile = flow.flowAtSmooth(x.toFloat(), y.toFloat())
            var flag = 0
            ...
            val dx = if (tile.reliable) tile.dx else 0f
            val dy = if (tile.reliable) tile.dy else 0f
            val centerX = x + dx
            val centerY = y + dy
            ...
            if (reference.rail[o] || moving.rail[warpQuadY * width + warpQuadX]) {
                raw[o] = 0f
                flags[o] = FLAG_SATURATED
                continue
            }
            if (tile.reliable && tile.residual > config.maxMeanAbsoluteResidual) {
                raw[o] = 0f
            ...
            if (!reference.modelValid || !moving.modelValid) {
                ...
                value = if (distance == 0.0) 1f else 0f
                if (distance != 0.0) flag = FLAG_PHOTO_CONFLICT
            } else {
                val irregular = flowIrregular(flow, x, y, motionThreshold)
                val scale = if (!tile.reliable || irregular) tuning.s1.toFloat() else tuning.s2.toFloat()
                value = (scale * exp(-(distance / variance).toFloat()) - threshold).coerceIn(0f, 1f)
    ...
    internal fun flowIrregular(flow: RawSrAlignmentField, x: Int, y: Int, motionThreshold: Float): Boolean {
        ...
        for (i in -1..1) for (j in -1..1) {
            ...
            if (!tile.dx.isFinite() || !tile.dy.isFinite()) return true
            finite = true
            minX = minOf(minX, tile.dx)
            minY = minOf(minY, tile.dy)
            maxX = maxOf(maxX, tile.dx)
            maxY = maxOf(maxY, tile.dy)
        }
