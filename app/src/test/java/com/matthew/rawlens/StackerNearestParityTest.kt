 ...
 * emission, the saturation guard, Double (not float) arithmetic, and
...
class StackerNearestParityTest {
    ...
    private fun frame(
        ...
        return RawSrBayerMerge.MergeFrame(w, h, samples, pattern, ox, oy, precision, flow, robustness)
    }
    ...
    @Test fun tieBreakFollowsOyOuterOxInnerOrder() {
        ...
        val w = 4
        val h = 4
        val samples = FloatArray(w * h) { i -> (i + 1).toFloat() }
    ...
    @Test fun fallbackBlendMatchesStackerBurstQuotient() {
        // Full Stacker burst semantics end to end: reference at unit weight
        // plus a moving frame at r = 0.1, rc forcing fallback everywhere.
        // Every channel must equal the transcribed
        // (w_mov * pick_mov + 1 * pick_ref) / (w_mov + 1) quotient bit-exactly,
        // including the production accumulation order (moving first, ref last).
        // Zero flow keeps both sources exactly at texel centers + 0.5, so no
        // float-blend ulp can perturb the pick geometry (shifted-pick coverage
        // lives in nearestPickMatchesStackerTranscription; oob-skip coverage
        // in outOfBoundsMovingFrameContributesNothing).
        val w = 8
        val h = 8
        ...
        assertTrue(out.fallback.all { it })
        val channelOf: (Int, Int) -> Int =
            { x, y -> channelIndex(BayerPattern.RGGB.colorAt(x, y)) }
        val wd = movWeight.toDouble()
        for (y in 0 until h) for (x in 0 until w) {
            val sx = x + 0.5
            val sy = y + 0.5
            for (c in 0..2) {
                val refPick = stackerNearest(refSamples, w, h, channelOf, c, sx, sy)
                val movPick = stackerNearest(movSamples, w, h, channelOf, c, sx, sy)
                assertNotNull("($x,$y,$c) ref pick", refPick)
                assertNotNull("($x,$y,$c) mov pick", movPick)
                var num = wd * movPick!!
                var den = wd
                num += 1.0 * refPick!!
                den += 1.0
                val expected = (num / den.coerceAtLeast(RawSrBayerMerge.EPS)).toFloat()
                assertEquals("($x,$y,$c)", expected, out.rgb[(y * w + x) * 3 + c], 0f)
            }
        }
    }
}
