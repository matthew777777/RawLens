...
6-coefficient by sensor color); R/B use their single sample's pair, G uses the
mean of its two samples' pairs:

- `α_c = mean(S/(W−b))`, `β_c = mean((S·b+O)/(W−b)²)` over the channel's samples.

Per-sample rail gate (code domain, using the sample's own `(S, O, b, W)`):

- `σ_code = sqrt(S·code + O)`; sample is near-rail iff
  `code > W − 3·σ_code` or `code < b + 3·σ_code`.
- Quad `q` is saturated iff any of its four samples is near-rail. The `3σ`
  multiplier is a 4C-chosen, test-validated constant (see §8): it replaces the
  reference clipped-statistics Monte Carlo LUT, which we do not implement.
...
| `SATURATED` | Near-rail samples in either frame's quad |
...
4. Analytic expected variance + rail rejection instead of Monte Carlo
   clipped-statistics LUT (§5); `3σ` rail multiplier is the single 4C-chosen
   constant, validated by saturation tests.
