...
def main():
    ...
    import re as _re
    def sensor_ns(path):
        return int(_re.search(r"_T(\d+)_", path.name).group(1))
    files = sorted(args.source.glob("*.dng"), key=sensor_ns)
    assert 2 <= len(files) <= 30, "stage at most 30 frames (fixture schema limit)"
    ...
    manifest = dict(schemaVersion=1, id="forest", dataKind="real-camera-bayer",
        license=dict(spdx="CC-BY-4.0", url="https://creativecommons.org/licenses/by/4.0/",
            attribution="RawLens user (forest GCam ZSL burst contributor)",
            permission="Contributor confirmed ownership and requested CC-BY-4.0 redistribution on 2026-09-11",
            changes="Lossless rectangular Bayer extraction and gzip packing only; no normalization, demosaic, alignment, or fusion"),
        source=dict(description="User-supplied forest GCam ZSL burst (HDR+ port tuning on 25080RABDG), oldest 30 of 32 frames",
            originalFrameCount=len(files),
            limitations=["Camera2 HAL row/pixel strides, camera/physical IDs and physical sensor-buffer origin unavailable in GCam payloads.",
                "Sensor timestamps are GCam filename T-field nanoseconds (ZSL sensor clock, ~33 ms cadence), not Camera2 SENSOR_TIMESTAMP.",
                "Coordinates below are the DNG active-array domain; physical HAL origin is unknown.",
                "DNG stored orientation needs 90-degree rotation for display; fixture uses unrotated active-array coordinates."]),
    ...
    for index, file in enumerate(files):
        ...
        with tifffile.TiffFile(file) as tif:
            ...
            manifest["frames"].append(dict(file=filename, sha256=sha(data), compressedSha256=sha(compressed),
                ...
                timestamp=dict(sensorNanos=sensor_ns(file), filenameMillis=sensor_ns(file),
                    dngModifyDate=exif["ModifyDate"], provenance="GCam payload filename T-field nanoseconds in filenameMillis for strict ordering (ZSL sensor clock, ~33 ms cadence); DNG ModifyDate"),
